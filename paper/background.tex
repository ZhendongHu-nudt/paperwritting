...
Distillation-based CIL methods (LwF~\cite{li2017lwf}, iCaRL~\cite{rebuffi2017icarl}) use a globally weighted preservation objective. The fixed weight cannot adapt to how compatible each flow is with previously learned class representations: flows close to and far from the historical-class manifold are regulated in the same way. Distillation can therefore preserve average historical behavior while overlooking flow-level differences in the required stability--plasticity balance.
