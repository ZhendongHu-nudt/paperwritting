...
% Topic sentence 1: claim
Each of RAR's three design choices contributes a measurable share of the overall gain; removing any one reopens part of the forgetting gap. We present the NUDT-Mobile results in the main text, and the same pattern holds across all six datasets.
% TODO: Add an appendix reference here after inserting the six-dataset ablation table.

Table~\ref{tab:ablation} reports component-level ablations on NUDT-Mobile. Removing the reconstruction routing signal raises forgetting by 5.78\,pp; reverting to a single head raises forgetting by 4.49\,pp and drops mix by 9.77\,pp; removing calibration raises forgetting by 3.41\,pp. These completed results quantify the effect of each component as implemented in the complete framework; the routing ablation removes both loss weighting and logit gating and therefore does not attribute the gain to either use in isolation. The constant-routing control retains both uses but replaces the per-flow score with $s_{\mathrm{old}}=s_{\mathrm{new}}=0.5$; its run is pending in this draft.

\begin{table}[t]
\caption{Ablation results on NUDT-Mobile; \emph{TBD} marks runs in progress.}
\label{tab:ablation}
\centering
\small
\begin{tabular}{lcc}
...
constant routing  & \textit{TBD} & \textit{TBD} \\
