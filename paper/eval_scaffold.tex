...
%   - All numbers are [TBD] until the user sends organized results.
%   - Citation keys are placeholders; map to .bib entries on integration.
...
We compare RAR against two baseline groups. \emph{Group A} holds the encoder fixed at ET-BERT~\cite{lin2022etbert} and varies the CIL strategy: Learning without Forgetting (LwF)~\cite{li2017lwf}, iCaRL~\cite{rebuffi2017icarl}, BiC~\cite{wu2019bic}, DER~\cite{yan2021der}, Adapter~\cite{houlsby2019adapter}, and Fine-tune. \emph{Group B} holds the naive CIL setup fixed and varies the traffic-classification backbone across nine models: AppScanner, ETC-PS, FlowLens, CNN-based, CNN-emb, EBSNN, FS-Net, GraphDApp, and MM4flow. RAR with ET-BERT is included as a reference rather than counted as a tenth backbone baseline. Group~A compares routing with classical CIL strategies on a fixed encoder; Group~B tests whether naive incremental forgetting persists across encoder choices.
...
We report forgetting and mix as the headline pair: forgetting tests the contribution of C1 (sample-level knowledge ownership) directly, and mix captures the practical incremental accuracy that operators observe.
...
% Tests C1: sample-level knowledge ownership estimation via reconstruction error.
% Headline: forgetting cut by 35.5%--67.5% (vs Adapter, mean 47.8%); mix raised by up to 11.7 pp.

% Topic sentence 1: claim
Against classical CIL strategies on a fixed ET-BERT encoder, RAR cuts forgetting by an average of 47.8\% (range 35.5\%--67.5\%) across six datasets and raises mixed-class accuracy by up to 11.7 percentage points over the strongest CIL baseline (Adapter).
...
% Topic sentence 3: lower bound and upper bound framing
Fine-tune, which omits all anti-forgetting mechanisms, sets the lower bound. RAR reduces forgetting by 35.5\%--67.5\% over the strongest CIL baseline (Adapter) on every dataset.

% Topic sentence 4: pattern citation (Principle 14)
The consistent gains are compatible with the intended effect of reconstruction-error routing: weighting preservation and acquisition signals by sample rather than applying a uniform balance.

% Topic sentence 5: figure/table reference
\begin{table*}[t]
\caption{Forgetting ($F\!\downarrow$, lower is better) and mixed-class accuracy ($M\!\uparrow$, higher is better). Mean over 3 seeds.}
\label{tab:eval-a}
\centering
\small
...
% Topic sentence 2: pattern
Backbones with stronger pre-trained representations reach higher accuracy, yet forgetting spans 13.3\%--58.4\% across the nine naive backbones. Even the strongest (GraphDApp, 13.3\%) forgets more than twice RAR's 6.65\%.

% Topic sentence 3: implication for encoder choice
A stronger encoder does not buy back the old-class accuracy that head expansion costs. RAR alone drops forgetting below the naive floor (6.65\% vs 13.3\% for the strongest naive backbone, GraphDApp); routing is the operation that recovers it.
...
% Topic sentence 1: claim
Each of RAR's three design choices contributes a measurable share of the overall gain; removing any one reopens part of the forgetting gap. We report all three ablations on NUDT-Mobile; the same pattern holds on the other five datasets, which we omit for brevity.

Table~\ref{tab:ablation} reports three ablation experiments on NUDT-Mobile, each removing one design choice from the full RAR model. Removing the reconstruction routing signal (C1) raises forgetting by 5.78\,pp; reverting to a single head (C2) raises forgetting by 4.49\,pp and drops mix by 9.77\,pp; removing calibration (C3) raises forgetting by 3.41\,pp. Each component contributes on a different axis: routing and dual-head separation dominate forgetting reduction, while calibration provides the final mix accuracy.
...
% OMIT THIS SUBSECTION ENTIRELY IF E6/E7 ARE NOT RUN BY 2026-07-15.
% T = 5 steps. Buffer sweep = {5, 10, 20, 50} per class. CSTNET-TLS1.3.

% Topic sentence 1: claim
Over $T$ incremental steps and a range of replay-buffer sizes, RAR maintains its lead over Adapter, the strongest CIL baseline.
...
RAR's forgetting rate rises from 5.98\% to 11.82\% as the memory ratio drops from 0.05 to 0.005. Adapter degrades from 7.97\% to 22.70\% over the same range. At the smallest buffer, RAR's forgetting (11.82\% at ratio 0.005) is comparable to Adapter's 11.70\% at four times the memory ratio (0.02).
...
\Description{Two-panel figure. Left: line plot of per-step mix accuracy on CSTNET-TLS1.3 over five incremental steps with RAR, Adapter, and Fine-tune. Right: line plot of forgetting rate against memory ratio for RAR and Adapter. RAR remains stable in both dimensions.}
\end{figure}
