...
%   reconstruction-based old/new compatibility -> soft routing
...
RAR's design follows a single line: reconstruction-based old/new compatibility, soft routing, dual knowledge paths, and calibration. A reconstruction autoencoder over frozen-encoder features produces a per-sample old-versus-new routing score. RAR uses that score to weight a frozen prediction path inherited from the preceding step and a trainable head for the arriving classes. Logit calibration then maps the two outputs into a single classification space. The rest of this section defines each component and the assumptions that bound the mechanism.
...
% -----------------------------------------------------------------
% 3.2 Reconstruction-based old/new compatibility
% -----------------------------------------------------------------
\subsection{Reconstruction-Based Old/New Compatibility}\label{sec:knowledge}

% Topic sentence 1: claim
RAR measures each sample's compatibility with the replay-derived old-class manifold using a scalar routing score. This score is an operational weight, not a calibrated posterior probability or a claim that knowledge belongs exclusively to one path.
...
We apply the routing signal in log space. Let $\ell_{\mathrm{old}}(x) \in \mathbb{R}^{K_0}$ and $\ell_{\mathrm{new}}(x) \in \mathbb{R}^{K_{\mathrm{new}}}$ be the raw logit vectors from the two heads. The gated logits are computed by adding the log of each head's routing score:
\begin{align}
