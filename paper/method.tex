...
%   knowledge ownership estimation -> soft routing
%   -> dual knowledge paths -> calibration.
...
RAR's design follows a single line: knowledge ownership estimation, soft routing, dual knowledge paths, and calibration. A reconstruction autoencoder over frozen-encoder features produces a per-sample old-versus-new routing score. RAR uses that score to weight a frozen prediction path inherited from the preceding step and a trainable head for the arriving classes. Logit calibration then maps the two outputs into a single classification space. The rest of this section defines each component and the assumptions that bound the mechanism.

\begin{figure*}[t]
\centering
\input{figures/fig_rar_architecture}
\caption{RAR architecture overview.}
\label{fig:rar-architecture}
\Description{Architecture diagram of RAR. Input flows through a frozen ET-BERT encoder into feature vector f(x). Three parallel branches: a frozen old-knowledge classification head, a trained new-knowledge head with adapter, and a reconstruction autoencoder branch that maps error through sigmoid into routing scores. Log-space gating combines the two head outputs, followed by scalar calibration into joint logits.}
\end{figure*}
...
Let $f: \mathcal{X} \to \mathbb{R}^d$ be a frozen encoder pre-trained on large-scale encrypted traffic data (ET-BERT~\cite{lin2022etbert}). At the base stage, we train a classifier $F_0$ on $K_0$ classes using $\mathcal{D}_0 = \{(x_i,y_i)\}_{i=1}^{N_0}$. At increment $t$, $K_t$ classes arrive with data $\mathcal{D}_t$, and a replay buffer $\mathcal{B}_{t-1}$ represents the classes seen before $t$. RAR freezes the complete preceding classifier $F_{t-1}$ as the old prediction path and learns a new-class head $h_t:\mathbb{R}^d\to\mathbb{R}^{K_t}$. For the single-increment setting, $F_{t-1}=F_0$ reduces to the frozen base classifier. Each sample is routed between $F_{t-1}$ and $h_t$ by a weight derived from reconstruction error. The following subsections define the reconstruction signal (\S\ref{sec:knowledge}), routing (\S\ref{sec:routing}), calibration (\S\ref{sec:calib}), and training procedure (\S\ref{sec:training}).

% -----------------------------------------------------------------
% 3.2 Knowledge ownership estimation
% -----------------------------------------------------------------
\subsection{Knowledge Ownership Estimation}\label{sec:knowledge}

% Topic sentence 1: claim
RAR represents each sample's location in the old-versus-new knowledge space by a scalar routing score derived from reconstruction error. This score is an operational routing weight, not a calibrated posterior probability.
...
The score does not require a class label at inference: it measures compatibility between $f(x)$ and the feature manifold learned by $g_\phi$. Its fitting stage does use the known old/new split between replay and newly arriving reference samples. This signal complements output-based distillation (LwF~\cite{li2017lwf}, iCaRL~\cite{rebuffi2017icarl}) and stored-example replay (DER~\cite{yan2021der}) by providing an input-dependent weight for balancing the two prediction paths.
...
\begin{figure*}[t]
\centering
\input{figures/fig_recon_routing}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_e0_separability.png}
...
% -----------------------------------------------------------------
% 3.2 Soft routing and dual knowledge paths 
% -----------------------------------------------------------------
\subsection{Soft Routing and Dual Knowledge Paths}\label{sec:routing}

% Topic sentence 1: claim
RAR routes each input through two knowledge paths: the frozen preceding classifier $F_{t-1}$ and a trainable new-class head $h_t$ equipped with a lightweight adapter. Their outputs are weighted by $s_{\mathrm{old}}(x)$ and $s_{\mathrm{new}}(x)$ from Eqs.~\eqref{eq:score-old}--\eqref{eq:score-new}.
...
We apply the routing signal in log space. Let $\ell_{\mathrm{old}}(x) \in \mathbb{R}^{K_0}$ and $\ell_{\mathrm{new}}(x) \in \mathbb{R}^{K_{\mathrm{new}}}$ be the raw logit vectors from the two heads. The gated logits are computed by adding the log of each head's ownership score:
\begin{align}
...
After incremental training, we freeze both prediction paths and refine $(b_{\mathrm{old}}, c_{\mathrm{new}}, b_{\mathrm{new}})$ on a balanced mixed calibration set by minimizing cross-entropy. The old portion is sampled from replay memory; the new portion is held out from the current increment.
...
First, replay memory should provide representative coverage of previously learned knowledge. If replay samples fail to capture the old-class distribution, the reconstructor may learn an incomplete representation of old knowledge, leading to inaccurate ownership estimation. \S\ref{sec:eval-buffer} evaluates different replay buffer sizes and shows that RAR remains robust even under a memory ratio of 0.005.

Second, reconstruction error should exhibit sufficient separability between old and emerging knowledge. When old and new classes have highly similar feature representations, reconstruction errors may overlap, reducing the reliability of knowledge ownership estimation. Figure~\ref{fig:e0-separability} shows the observed separation across the six evaluated datasets.

These assumptions characterize the applicability of RAR rather than strict limitations. RAR is most effective when replay samples preserve representative old knowledge and when old and emerging knowledge exhibit distinguishable representation patterns.
